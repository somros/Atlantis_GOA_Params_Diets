...
predator <- predator %>% filter(Year>=1990)
prey_length <- prey_length %>% filter(Year>=1990)
...
calculate_diet_prop <- function(pred_stage,prey_stage){
  ...
  # write this out
  write.csv(diet_all_years,paste("../output/REEM/dietprop_stratum_pred",pred_stage,"prey",prey_stage,".csv",sep = "_"), row.names = F)
  ...
  ggsave(paste("../output/REEM/dietprop_stratum_pred",pred_stage,"prey",prey_stage,".png",sep = "_"),p,width = 16, height = 12)
}
