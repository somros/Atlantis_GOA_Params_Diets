...
nodc_key_native <- read.csv("data/PreyNODCForWeb.csv")
```
...
```{r}
nodc_key1 <- read.csv("data/nodc_key1.csv")
nodc_key1 <- nodc_key1 %>% select(-Nodc,-Name1)

nodc_key1 <- nodc_key1 %>% left_join(nodc_key_native, by = "Name")

# now bring in the Atlantis groups
nodc_key1 <- nodc_key1 %>% left_join(goa_groups %>% select(Code,Name,LongName), by = c("Atlantis.group" = "Code"))
nodc_key1 <- nodc_key1 %>% na.omit()

nodc_matching_key <- nodc_key1
save(nodc_matching_key, file = "data/nodc_matching_key.Rdata")
```
