---
title: "Ontogenetic diet preferences - Atlantis groups"
...
9. Apply the steps from diet_exploration_atlantis_groups_all_indeces.Rmd to calculate weight-based preference for a prey species, weighted by the number of predators in a haul. Do this 4 times for all the combination of juvenile and adult predators and prey. This should yield four matrices for vertebrates. Biomass pools will only have two matrices. At this stage, before calclating the proportions, map the weight of ingested prey to Atlantis groups by adding appropriately. 
...
**NOTE**: there are other ways of doing this. Kerim's previous efforts use length composition of prey species in the stomachs of each predator species at stratum level, then use the proportions (in our case juveniles vs adults) in that stratum to break down the weight of ingested prey for that predator at haul level. That dampens inter-haul variability in the size comp of the prey. Kirstin has recommended to avoid working at haul level, because hauls (and subsamples within them) are **not representative**. Worth bearing in mind that diet input in Atlantis is not spatially explicit. 
...
calculate_diet_prop <- function(pred_stage,prey_stage){
  ...
  # make a counter for the number of predators aggregated by Atlantis group
  ...
  # add new counter to the predator set
  ...
  ggsave(paste("dietprop_pred",pred_stage,"prey",prey_stage,".png",sep = "_"),p,width = 16, height = 12)
}
